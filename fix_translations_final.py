    ...
    while i < len(lines):
        ...
        if line.startswith('msgid '):
            ...
            if line == 'msgid ""':
                ...
                # 单行msgid
                msgid_lines.append(line[6:])  # 去掉 'msgid '
                i += 1
            ...
            if i < len(lines) and lines[i].strip().startswith('msgstr'):
                ...
                if msgstr_line == 'msgstr ""':
                    ...
                    # 单行msgstr
                    msgstr_lines.append(msgstr_line[7:])  # 去掉 'msgstr '
                    i += 1
            
            # 提取msgid内容
            msgid = extract_string(msgid_lines)
            msgstr = extract_string(msgstr_lines)
...
def fix_translations(input_file, output_file):
    ...
    for entry in reversed(entries):
        if entry['msgstr'] == '' and entry['msgid'] in TRANSLATIONS:
            ...
            new_msgstr_lines = format_msgstr(translation)
            
            # 找到msgstr开始的位置
            msgid_end = entry['start'] + len(entry['msgid_lines'])
            if entry['msgid_lines'] and entry['msgid_lines'][0].startswith('msgid'):
                msgid_end = entry['start'] + 1 + len(entry['msgid_lines'])
            
            # 替换msgstr行
            old_msgstr_count = len(entry['msgstr_lines'])
            new_msgstr_count = len(new_msgstr_lines)
            ...
            if msgstr_start < len(lines):
                # 删除旧的msgstr行
                for _ in range(old_msgstr_count):
                    if msgstr_start < len(lines):
                        lines.pop(msgstr_start)
                
                # 插入新的msgstr行
                for j, new_line in enumerate(new_msgstr_lines):
                    lines.insert(msgstr_start + j, new_line)
