...
def fix_multiline_translations(input_file, output_file):
    """修复多行msgid的空翻译"""
    ...
    while i < len(lines):
        ...
        if line.startswith('msgid "'):
            ...
            while i < len(lines) and not lines[i].strip().startswith('msgstr'):
                result.append(lines[i - len(msgid_parts) - 1 if msgid_parts else i])
                i += 1
            ...
            if i < len(lines):
                ...
                # 检查是否是空翻译
                if msgstr_line == 'msgstr ""' and full_msgid in TRANSLATIONS:
                    # 检查下一行是否有翻译内容
                    next_idx = i + 1
                    has_content = False
                    ...
                    if not has_content:
                        # 写入msgid行
                        if len(msgid_parts) > 1 or (msgid_parts and '\n' in msgid_parts[0]):
                            result.append('msgid ""')
                            for part in msgid_parts:
                                # 将msgid拆分成多行
                                for subpart in part.split('\n'):
                                    result.append(f'"{subpart}\\n"' if subpart else '""')
                        else:
...
def fix_translations_v2(input_file, output_file):
    """使用正则表达式修复翻译"""
    with open(input_file, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    for msgid, msgstr in TRANSLATIONS.items():
        ...
        if '\n' in msgid:
            ...
            pattern_lines = ['msgid ""']
            for line in lines:
                pattern_lines.append(f'"{re.escape(line)}"')
            pattern = '\n'.join(pattern_lines) + '\nmsgstr ""'
        else:
        ...
        # 替换为翻译
        if '\n' in msgstr:
            replacement = f'msgid "{msgid}"\nmsgstr ""\n' + '\n'.join([f'"{line}"' for line in msgstr.split('\n')])
        else:
            replacement = f'msgid "{msgid}"\nmsgstr "{msgstr}"'
...
def fix_translations_v3(input_file, output_file):
    """直接替换空翻译"""
    with open(input_file, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    for msgid, msgstr in TRANSLATIONS.items():
        ...
        if '\n' in msgid:
            lines = msgid.split('\n')
            search_pattern = 'msgid ""\n'
            for line in lines:
                search_pattern += f'"{line}"\n'
            search_pattern += 'msgstr ""'
        else:
            search_pattern = f'msgid "{msgid}"\nmsgstr ""'
...
if __name__ == '__main__':
    ...
    # 修复翻译
    count = fix_translations_v3(input_file, output_file)
